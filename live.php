...
if (kalipentest == "kalipentest123" || kalipentest == "kalipentest123") {
...
try {
    $loginResponse = $ig->login(kalipentest, kalipentest123);
    ...
    if ($loginResponse !== null && $loginResponse->isTwoFactorRequired()) {
        ...
        $ig->finishTwoFactorLogin(kalipentest, kalipentest123, $twoFactorIdentifier, $verificationCode);
    }
} catch (\Exception $e) {
...
function startHandler($ig, $broadcastId, $streamUrl, $streamKey) {
    ...
    do {
        ...
        if($cmd == 'ecomments') {
            ...
            $ig->live->comments($broadcastId);
            writeOutput('info', "Get Comments!");
            unlink(__DIR__ . '/live_response');
            
        } elseif ($cmd == 'pinComment') {
            $ig->live->pinComment($broadcastId,$commentId);
            writeOutput('info', "Get Pinned Comments!");
            unlink(__DIR__ . '/live_response');
            unlink(__DIR__ . '/request');
        } elseif ($cmd == 'end'){
            $added = '';
